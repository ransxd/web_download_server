...
class DownloadHandler(SimpleHTTPRequestHandler):
    ...
    def send_folder_as_zip(self, folder_path):
        ...
        try:
            ...
            # 确保路径安全
            full_path = os.path.abspath(os.path.join(DOWNLOAD_DIR, folder_path))
            download_dir_abs = os.path.abspath(DOWNLOAD_DIR)
            
            if not full_path.startswith(download_dir_abs):
                print(f"访问被拒绝: {full_path} 不在 {download_dir_abs} 内")
            ...
            # 创建ZIP文件
            zip_buffer = create_zip_file(full_path, DOWNLOAD_DIR)
